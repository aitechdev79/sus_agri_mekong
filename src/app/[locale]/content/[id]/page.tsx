...
function formatEventRange(content: PublicContent, locale: string) {
  ...
  if (content.isAllDay) {
    return formatVietnamDate(start, locale);
  }

  const startLabel = formatVietnamDateTime(start, locale);
  ...
  const endLabel = formatVietnamDateTime(end, locale);
              ...
              {hasPdf ? (
                hasRichTextContent && (
                  <section>
                    ...
                    <div
                      className="prose prose-lg max-w-none text-justify prose-headings:text-gray-900 prose-headings:mb-4 prose-headings:mt-8 prose-p:leading-relaxed prose-p:text-gray-800 [&_p]:mt-0 [&_p]:mb-2 [&_p:has(br:only-child)]:my-4 [&_p:has(br:only-child)]:leading-none [&_ul]:list-disc [&_ul]:list-inside [&_ul]:pl-2 [&_ol]:list-decimal [&_ol]:list-inside [&_ol]:pl-2 [&_li]:my-1 [&_li>p]:inline [&_li>p]:m-0 [&_blockquote]:my-6 [&_blockquote]:border-l-4 [&_blockquote]:border-green-700 [&_blockquote]:bg-green-50 [&_blockquote]:px-5 [&_blockquote]:py-4 [&_blockquote]:italic [&_blockquote_p]:text-gray-800"
