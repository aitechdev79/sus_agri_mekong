...
export default function HeroSectionAPI() {
  ...
  // Use API routes for images with fallbacks
  const getImageSrc = () => {
    if (imageError) {
      return 'https://images.unsplash.com/photo-1500595046743-cd271d694d30?w=1920&h=1080&fit=crop&crop=center&auto=format&q=80';
    }

    // Try API route first
    if (isMobile) {
      return '/api/static/hero/hero-mobile.jpg';
    } else {
      return '/api/static/hero/hero-main.jpg';
    }
  };
    ...
    >
      ...
      >
        ...
        <Image
          ...
          onError={() => {
            console.error('Hero image failed to load via API route, using external fallback');
            setImageError(true);
          }}
        />
      </div>
