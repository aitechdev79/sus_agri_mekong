...
export default function NewsSection() {
  ...
  const featuredEvents =
    upcomingEvents.length >= 2 ? upcomingEvents.slice(0, 2) : [...upcomingEvents, ...pastEvents.slice(0, 2 - upcomingEvents.length)];
      ...
      <section className="py-16 w-full">
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="animate-pulse">
            <div className="h-8 bg-white/20 rounded w-1/3 mb-8" />
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white/20 rounded-lg h-96" />
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        <div className="mb-12 flex items-center justify-between">
          ...
          <Link
            ...
            style={{ color: '#3C3C3B', boxShadow: '0 4px 12px rgba(255, 193, 7, 0.15)' }}
          >
            {isEn ? 'View all events' : 'Xem tất cả sự kiện'}
                ...
                >
                  <div className="flex flex-row items-center px-6 py-4 transition-transform duration-500 group-hover:scale-105" style={{ minHeight: '120px' }}>
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                      ...
                      <h3 className="text-lg md:text-xl font-bold mb-2 font-montserrat line-clamp-2" style={{ color: '#3C3C3B' }}>{title}</h3>
                      {description && <p className="text-sm md:text-base line-clamp-2 font-montserrat" style={{ color: '#6B7280' }}>{description}</p>}
                      {item.eventLocation && <p className="mt-2 text-sm font-montserrat" style={{ color: '#6B7280' }}>{item.eventLocation}</p>}
                    </div>
                  </div>
                </Link>
              );
