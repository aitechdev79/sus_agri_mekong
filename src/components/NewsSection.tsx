                    ...
                    <div className="relative h-64 md:h-80 overflow-hidden">
                      {(() => {
                        ...
                        if (item.thumbnailUrl && item.thumbnailUrl.includes('/uploads/')) {
                          ...
                          // Fallback: try to find uploaded images that might match this content
                          // Look for thumbnail files that might belong to this content
                          const possibleImages = [
                            `/uploads/thumb_${item.id}.jpg`,
                            `/uploads/${item.id}.jpg`,
                            `/uploads/thumb_${item.id}.png`,
                            `/uploads/${item.id}.png`,
                            // Use one of the existing uploaded images as fallback
                            '/uploads/featured.jpg'
                          ];
                          imageUrl = possibleImages[0]; // Default to first possibility
                        }
