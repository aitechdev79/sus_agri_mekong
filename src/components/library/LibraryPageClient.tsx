...
import { PublicListPagination, LibraryListItem } from '@/lib/public-content-list'
...
function formatDate(dateString: string, locale: string) {
  return formatVietnamDate(dateString, locale === 'en' ? 'en' : 'vi')
}

interface LibraryPageClientProps {
  locale: string
  initialContents: LibraryListItem[]
  initialPagination: PublicListPagination
...
export default function LibraryPageClient({
  ...
  const [contents, setContents] = useState<LibraryListItem[]>(initialContents)
    ...
    () =>
      Object.fromEntries(
        initialCategories.map((category) => [
          category.slug,
          isEn ? (category.nameEn || category.name || category.nameVi) : (category.nameVi || category.name),
        ]),
      ),
  ...
  useEffect(() => {
    ...
    loadContents()
  }, [initialLoadComplete, loadContents])
                      ...
                      >
                        <div className="flex flex-col gap-3 md:grid md:grid-cols-[72px_minmax(0,1fr)_160px_140px] md:items-center md:gap-4">
                          ...
                          <div className="font-montserrat text-sm text-[#6B7280]">
                            {formatDate(String(content.publishedAt || content.createdAt), locale)}
                          </div>
                        </div>
                      </Link>
                    )
